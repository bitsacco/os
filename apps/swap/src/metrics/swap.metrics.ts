...
export class SwapMetricsService extends OperationMetricsService {
  private readonly logger = new Logger(SwapMetricsService.name);
  ...
  private buyRateObservable!: Observable<number>;
  private sellRateObservable!: Observable<number>;
  ...
  private initializeMetrics() {
    ...
    // Onramp histograms
    this.onrampAmountKesHistogram = this.createHistogram('swap.onramp.amount_kes', {
      description: 'Distribution of onramp amounts in KES',
      unit: 'KES',
    });

    this.onrampAmountSatsHistogram = this.createHistogram('swap.onramp.amount_sats', {
      description: 'Distribution of onramp amounts in satoshis',
      unit: 'sats',
    });

    this.onrampDurationHistogram = this.createHistogram('swap.onramp.duration', {
      description: 'Duration of onramp transactions',
      unit: 'ms',
    });

    // Offramp histograms
    this.offrampAmountKesHistogram = this.createHistogram('swap.offramp.amount_kes', {
      description: 'Distribution of offramp amounts in KES',
      unit: 'KES',
    });

    this.offrampAmountSatsHistogram = this.createHistogram('swap.offramp.amount_sats', {
      description: 'Distribution of offramp amounts in satoshis',
      unit: 'sats',
    });

    this.offrampDurationHistogram = this.createHistogram('swap.offramp.duration', {
    ...
    // FX rate observables
    this.buyRateObservable = this.createObservable<number>(
      'swap.fx.buy_rate',
      {
    ...
    this.sellRateObservable = this.createObservable<number>(
      'swap.fx.sell_rate',
      {
