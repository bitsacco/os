...
async function generateGlobalDevKey() {
  ...
  try {
    ...
    await withMongoClient(async (client) => {
      ...
      console.log(
        `Deleted ${deleteResult.deletedCount} existing global dev keys`,
      );
    ...
    for (const service of SERVICES) {
      ...
      if (fs.existsSync(envFilePath)) {
        ...
        console.log(
          `Warning: Environment file for ${service} not found: ${envFilePath}`,
        );
      }
    }
    ...
    console.log(
      'IMPORTANT: This key has access to ALL services. Do not use in production!',
    );
    console.log('======================================');
  } catch (error) {
