...
async function runDiagnostic() {
  ...
  console.log(
    'This script will help diagnose issues with API key authentication',
  );
  ...
  if (fs.existsSync(apiEnvPath)) {
    ...
    if (saltMatch) {
      ...
      if (configuredSalt && saltMatch[1] !== configuredSalt) {
        console.error(
          '⚠️ API service is using a different salt than root .dev.env!',
        );
      }
    } else {
  ...
  await withMongoClient(async (client) => {
    ...
    const globalKeys = await collection
      .find({
    ...
    if (globalKeys.length > 0) {
      ...
      if (configuredSalt && configuredSalt !== defaultSalt) {
        const configHash = crypto
          .createHmac('sha256', configuredSalt)
          .update(globalApiKey)
          .digest('hex');
        const configMatch = configHash === key.keyHash;
        console.log(
          `  - Configured salt: ${configMatch ? '✅ MATCH' : '❌ NO MATCH'}`,
        );
      }
      ...
      console.log(
        `  - Default salt query: ${matchDefault ? '✅ MATCH FOUND' : '❌ NO MATCH'}`,
      );

      const matchOld = await collection.findOne({ keyHash: oldHash });
      console.log(
        `  - Old salt query: ${matchOld ? '✅ MATCH FOUND' : '❌ NO MATCH'}`,
      );
    } else {
      console.log(
        '⚠️ No global API keys found. Using first available key for testing.',
      );
      ...
      if (allKeys.length > 0) {
        ...
        const oldMatch = oldHash === key.keyHash;
        console.log(
          `  - Old dev salt: ${oldMatch ? '✅ MATCH' : '❌ NO MATCH'}`,
        );
      }
    }
  });
  ...
  try {
    ...
    try {
      ...
      console.log(
        `  ✅ Public Health Endpoint Status: ${publicResponse.status}`,
      );
    } catch (publicError) {
      if (publicError.code === 'ECONNREFUSED') {
        console.error(
          '  ❌ API server is not running. Please start the API with "bun dev"',
        );
        console.log('\n⚠️ Diagnostic incomplete - API server is not running');
        return;
      } else {
        console.error(
          `  ⚠️ Public health endpoint error: ${publicError.message}`,
        );
      }
    }
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          ...
          console.log(
            '  - Check if API_KEY_SALT is set consistently in all services',
          );
          console.log(
            '  - Ensure the API service was restarted after updating the salt value',
          );
          console.log(
            '  - Verify API key in database has required scopes (should include service:api)',
          );
          console.log(
            '  - Try generating a new global key with "bun apikey:generate"',
          );
          console.log(
            '  - Check that API service can connect to MongoDB correctly',
          );
        }
      } else {
