...
import { Counter, Histogram, Observable } from '@opentelemetry/api';
...
export class CoreMetricsService extends MetricsService {
  ...
  // Resource metrics
  private cpuUsageObservable!: Observable<number>;
  private memoryUsageObservable!: Observable<number>;
  private diskUsageObservable!: Observable<number>;
  private networkRxObservable!: Observable<number>;
  private networkTxObservable!: Observable<number>;
  ...
  private initializeMetrics() {
    // Database metrics
    this.databaseOperationCounter = this.createCounter('core.database.operations', {
      description: 'Number of database operations',
    });

    this.databaseOperationDurationHistogram = this.createHistogram('core.database.duration', {
    ...
    this.apiRequestDurationHistogram = this.createHistogram('core.api.duration', {
    ...
    this.grpcRequestDurationHistogram = this.createHistogram('core.grpc.duration', {
    ...
    // Resource metrics
    this.cpuUsageObservable = this.createObservable<number>(
    ...
    this.memoryUsageObservable = this.createObservable<number>(
    ...
    this.diskUsageObservable = this.createObservable<number>(
    ...
    this.networkRxObservable = this.createObservable<number>(
    ...
    this.networkTxObservable = this.createObservable<number>(
  ...
  recordDatabaseMetric(metric: DatabaseMetric): void {
    ...
    this.metrics.database.averageDuration = 
      (this.metrics.database.averageDuration * (this.metrics.database.operations - 1) + metric.duration) / 
      this.metrics.database.operations;
  ...
  recordApiMetric(metric: ApiMetric): void {
    ...
    this.metrics.api.averageDuration = 
      (this.metrics.api.averageDuration * (this.metrics.api.requests - 1) + metric.duration) / 
      this.metrics.api.requests;
  ...
  recordGrpcMetric(metric: GrpcMetric): void {
    ...
    this.metrics.grpc.averageDuration = 
      (this.metrics.grpc.averageDuration * (this.metrics.grpc.requests - 1) + metric.duration) / 
      this.metrics.grpc.requests;
    ...
    switch (metric.resource) {
      ...
      case 'network':
        if (metric.unit === 'rx') {
          ...
          this.updateResourceStats(this.metrics.resources.network.rx, metric.value);
        } else if (metric.unit === 'tx') {
          this.currentResources.networkTx = metric.value;
          this.metrics.resources.network.tx.current = metric.value;
          this.updateResourceStats(this.metrics.resources.network.tx, metric.value);
        }
        break;
    }
   ...
   */
  private updateResourceStats(stats: { current: number; average: number; peak: number }, value: number): void {
    // Update peak
    if (value > stats.peak) {
      stats.peak = value;
    }
